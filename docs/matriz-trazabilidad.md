...
|---|---|---|---|---|---|
| Entregable 03 (mockups) · RF-012 evidencia visible | `docs/mockups/` — diecisiete pantallas en `.html` autocontenido y `.png`, generadas desde la aplicación real con `frontend/scripts/mockups.mjs`; `docs/entrega/puml/` con los 32 diagramas en PlantUML. El seed dejó de dar 410 al pedir una evidencia: escribe 1.126 ilustraciones sintéticas (`prisma/imagen-demo.ts`) | `npm run verificar:mockups`: abre los diecisiete `.html` desde `file://` **con la red bloqueada** y comprueba CSS embebido, cero `<script>`, todas las imágenes cargadas y el rojo heráldico de la barra | PASS (17/17, sin errores de JS) | A | ✅ |
