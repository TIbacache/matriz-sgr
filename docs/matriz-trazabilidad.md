...
**Actualizada**: 3 de septiembre de 2026 (Bloque B4 — atención social y sus tres gestiones) · **Se actualiza en cada cierre de sprint.**
...
| Épica | Historias | ✅ | 🟡 | ⬜ |
|---|---|---|---|---|
| EP-01 Registro y gestión de actividades | HU-01…04 | 3 | 1 | 0 |
...
| HU-02 Registro de compromisos ciudadanos | RF-016…021 | HU-3.2 | `91e8fcb` `b5e7eff` `backend/src/routes/tareas.routes.ts` · `frontend/src/components/NuevaTareaModal.tsx` | smoke-17, smoke-18 | PASS | A | 🟡 **falta solicitante, territorio, área de apoyo e INT/EXT, y con ellos el vínculo con el vecino**: los campos existen en el esquema y los llena el seed, pero el `tareaSchema` no los acepta, así que un compromiso externo creado desde la aplicación **no llega a la ficha del vecino** aunque `services/vecinos.ts` sí sepa leerlo. Es el Bloque B5 |
...
**278 comprobaciones, todas en verde** al 03-09-2026: **195 del backend** (18 smoke + 21 cálculo + 156 API) y **83 de contraste del frontend**. Aparte corre `npm run verificar:rut` (16 RUT del seed más los casos de normalización), que no entra en el total porque depende de que la base esté sembrada.
...
### `npm run verificar:api` — 156/156 (Bloques A, A2, A3, B, B2, B3 y B4)
